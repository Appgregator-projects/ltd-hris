...
export const uploadFileFirebase = async (
	...
	const storageRef = ref(storage, `course/${data.name}`);
...
export const uploadFile = async (title, type, file) => {
	const path = `${type}/${title}_800x800`;
	...
	try {
		...
		const thumbnailURL = `https://firebasestorage.googleapis.com/v0/b/lifetime-design-erp.appspot.com/o/${encodeURIComponent(
			thumbnailPath
		)}?alt=media`;
