...
const OvertimeRequest = () => {
  ...
  const getOvertime = async () => {
    ...
    if (dataUser?.title === "Manager") {
      conditions.push({ field: 'department_id', operator: '==', value: dataUser?.department_id })
    }
    ...
    try {
      ...
      // }
      const res = await Api.get(`/hris/overtimes?id=${dataUser?.employee_id}&status=${filterStatus}&startDate=${startDate}&endDaet=${endDate}`)
      if (res.status) {
  ...
  const onSubmitForm = async (payload) => {
    ...
    if (dataUser?.id === dataUser?.department?.manager_id) {
      const newData = {
        ...
        manager_id: dataUser?.department?.manager_id,
        status: 'process',
      ...
      const newDatas = {
        approvals: [{ id: dataUser?.department?.manager_id, name: 'manager' }],
        approved_hr: null,
